	...
	for entryIndex in 0 ..< entryCount {
		...
		for bus in entry.startBus ..= entry.endBus {
			...
			for offset := u64(0); offset < u64(mem.Megabyte); offset += shared.PAGE_SIZE {
				pmm.map_page(pmm.kernelPML4, busBase + offset, ._4KB,
			...
			for device in u8(0) ..< 32 {
				...
				for function in u8(0) ..< functionCount {
					header := kernel_pci_header(entry.baseAddr, entry.startBus, bus, device, function)
					if header.vendorId == pci.PCI_NO_DEVICE do continue
					barCount: u32 = 6
					if (header.headerType & 0x7F) == 0x01 do barCount = 2
					info := pci.Device {
						segment = entry.segmentGroup, bus = bus, device = device, function = function,
						vendorId = header.vendorId, deviceId = header.deviceId,
						classCode = header.classCode, subclass = header.subclass,
						progIf = header.progIf, configBase = u64(uintptr(header)),
					}
					index: u32 = 0
					for index < barCount {
						low := kernel_pci_bar_read(header, index)
						defer index += 1
						if low == 0 || (low & 1) != 0 do continue
						is64 := ((low >> 1) & 0x3) == 0x2 && (index + 1) < barCount
						high: u32 = 0
						if is64 do high = kernel_pci_bar_read(header, index + 1)
						bar := pci.Bar {
							addr = (u64(high) << 32) | u64(low & 0xFFFF_FFF0),
...
kernel_pci_bar_size :: proc(header: ^pci.Header, index: u32, is64: bool) -> u64 {
	...
	mask := u64(kernel_pci_bar_read(header, index) & 0xFFFF_FFF0)
